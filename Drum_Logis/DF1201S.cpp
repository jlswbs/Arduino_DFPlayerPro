/*!
 *@file DF1201S.cpp
 *@brief Define the basic structure of class DF1201S, the implementation of the basic methods
...
bool DF1201S::setVol(uint8_t vol)
{
   sPacket_t cmd;
...
DF1201S::ePlayMode_t DF1201S::getPlayMode()
{
...
bool DF1201S::setBaudRate(uint32_t baud)
{
   sPacket_t cmd;
...
bool DF1201S::switchFunction(eFunction_t function)
{
   sPacket_t cmd;
...
bool DF1201S::setPlayMode(ePlayMode_t mode)
{
...
bool DF1201S::setPrompt(bool on)
{
   sPacket_t cmd;
   String mode;
   if (on == true)
...
bool DF1201S::playSpecFile(String str)
{
   if (curFunction != MUSIC) return false;
   sPacket_t cmd;
...
bool DF1201S::playFileNum(int16_t num)
{
   if (curFunction != MUSIC) return false;
   sPacket_t cmd;
...
bool DF1201S::fastForward(uint16_t second)
{
   if (curFunction != MUSIC) return false;
   start();
...
bool DF1201S::fastReverse(uint16_t second)
{
   if (curFunction != MUSIC) return false;
   start();
...
bool DF1201S::setPlayTime(uint16_t second)
{
...
uint16_t DF1201S::getINT(String str)
{
   uint8_t numLen = 0;
   uint16_t num = 0;
...
uint16_t DF1201S::getCurFileNumber()
{
   if (curFunction != MUSIC) return false;
   sPacket_t cmd;
   cmd = pack("QUERY", "1");
...
uint8_t DF1201S::unicodeToUtf8(uint16_t unicode, uint8_t* uft8)
{
...
DF1201S::sPacket_t DF1201S::pack(String cmd, String para)
{
   sPacket_t pack;
   atCmd = "";
   atCmd += "AT";
...
void DF1201S::writeATCommand(String command, uint8_t length)
{
   uint8_t data[200];
   while (_s->available()) {
      _s->read();
...
String DF1201S::readAck(uint8_t len)
{
